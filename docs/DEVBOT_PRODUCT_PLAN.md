...
- Discord slash commands:
  ...
  - `/schedule add|list|remove|pause|resume` runs recurring owner-only **read-only** tasks (always `ask` — never write-capable, since nobody approves an unattended occurrence in the moment) on a simple `daily HH:MM` / `weekdays HH:MM` / `every <N>h` spec. Each tick atomically claims (leases) every due entry before executing it, so an occurrence still running past its 30-second check-in is never claimed again by a later tick; checked on a 30-second loop with restart-safe next-run math and a recoverable running lease if Devbot restarts mid-occurrence. Legacy write-capable schedule records are rejected on load and again at execution (fail closed), and safe mode blocks creating or resuming schedules.
