...
- Opt-in preview tunnels:
  ...
  - The exact loopback origin is independently re-validated (scheme, address, and port preserved exactly, never rewritten; remote hosts and default-port confusion rejected) rather than trusting only upstream detection. Tunnel URLs are posted embed-suppressed so the message itself never triggers a crawler.
  - Tunnels auto-expire (default 15, max 60 minutes), can be stopped early with `/preview stop` or a message button, and are always stopped (SIGTERM, escalating to SIGKILL if needed) on disable or Devbot shutdown. A kill that never confirms an exit is reported, not claimed successful. Owner `/preview stop`/`status` keep working even while the feature is disabled or safe mode is on; safe mode blocks new exposure only.
  - Every spawned `cloudflared` pid is recorded in a local ledger and released only on a confirmed exit; startup reconciles the ledger and kills orphans left by an unclean previous exit (only pids still identifiable as `cloudflared`, so recycled pids are never signalled). If a process cannot be verified or refuses to die, it stays in the ledger and is reported for manual cleanup.
  - `cloudflared` runs with an isolated, single-use `HOME` and XDG directory tree (removed after a verified exit) so it cannot pick up ambient Cloudflare account state, plus the bot's usual token/secret-stripped environment. `/setup doctor` validates that the found binary runs and identifies itself via `cloudflared --version`.
