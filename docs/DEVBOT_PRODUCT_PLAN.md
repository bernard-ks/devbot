...
- Video proof-of-work and live watch mode:
  ...
  - Recording interactions are restricted to demonstrably non-mutating actions: navigating document links (`a[href]`, excluding script-scheme targets), scrolling, and hovering. Buttons, submit inputs, and other write-capable controls are never clicked, so a read-level `/clip` cannot submit forms or trigger destructive app actions. Link steps navigate by loading the anchor's vetted same-origin target directly rather than dispatching a DOM click, so an anchor's own `onclick` handler never runs and cannot mutate application state before navigation.
  - `/clip` and completion recording confine the browser at `BrowserContext` scope before any page exists: request routing re-issues redirects as routed requests, service workers are blocked, popups and off-origin top-level documents are closed, and WebSockets are proxied so only approved loopback origins connect. Post-navigation URL checks and sanitized reported URLs (no credentials, query, or fragment) back this up.
  - Recording is bounded by real resource limits: one recording at a time, a hard abort deadline that closes the browser and times out `ffmpeg` regardless of where the flow is stuck, and a minimal child environment for the Chromium and `ffmpeg` processes.
