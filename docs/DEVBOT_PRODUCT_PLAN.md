...
- Agent-vs-agent duel review:
  ...
  - Results post to a dedicated thread with evidence coverage, snapshot identity, parser warnings, reviewer-independence status, and a resolved issue list (conceded / disputed). A durable, versioned duel record tracks the review lifecycle independently of the collaboration store, and clean, failed, and dismissed duels close their conversation terminally.
  - The exact reviewed state is pinned at duel time as a commit under a hidden `refs/devbot/duels/<duel-id>` ref (built through a temporary index overlay with the same sensitive-path exclusions and untracked-size bounds as the evidence, never mutating the working tree or the real index) and its identity is recorded with the duel; the capture is bound to the evidence by re-hashing the change set and discarded if the tree moved in between.
  - Stage 2 (**Accept & fix**) exists because its two review preconditions are met: the reviewed snapshot is reproducible (the hidden ref must still resolve to the exact recorded commit and tree, re-verified both when the control is rendered and again when it is used), and the decision-to-task transition is atomic and auditable (a compare-and-set claim on the durable duel record moves it through accepting to accepted with the resulting task ID persisted, or to failed-retryable with the claim released; duplicate or concurrent submissions get exactly one launch, and interrupted claims are released as retryable on restart).
  - An accepted fix task is seeded in a fresh isolated worktree created from the snapshot commit — never the source checkout or the original task worktree — and runs through the normal task path with the conceded issues as its input. Decision controls stay bound to the duel thread's control message, re-check controller status and project policy at claim time, and are blocked by safe mode; when the snapshot is missing or stale the duel falls back to a copyable fix prompt with a truthful explanation. Snapshot refs are deleted when their duels end without conceded issues, are dismissed, accepted, or pruned from retention, with a bounded total per repository.
