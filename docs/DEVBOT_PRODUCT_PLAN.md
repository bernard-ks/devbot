...
- Voice note to task (opt-in, `DEVBOT_VOICE_ENABLED=true`, off by default until smoke-tested end to end):
  - Requires the privileged Message Content Intent (`DEVBOT_MESSAGE_CONTENT_INTENT=true` plus MESSAGE CONTENT INTENT enabled in the Discord Developer Portal): voice attachments are delivered as Message Content data, which Discord withholds without the intent except for its documented exceptions (DMs, messages that @mention the bot, the bot's own messages, and context-menu targets), so a native voice note in the room would otherwise arrive with no attachment. Enabling voice without the intent is refused with an actionable setup message, and `/setup doctor` reports the intent prerequisite as its own READY/FIX line.
