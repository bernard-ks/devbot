...
- Agent-vs-agent duel review:
  ...
  - Results post to a dedicated thread with evidence coverage, snapshot identity, parser warnings, reviewer-independence status, and a resolved issue list (conceded / disputed). The duel is read-only in this stage: conceded issues get owner/controller **Copy fix prompt** (a copyable follow-up prompt, never an automatic write task) and **Dismiss** controls bound to that thread's message and gated by project policy; automated fix tasks return only once the reviewed snapshot is reproducible and the decision-to-task transition is atomic. A durable, versioned duel record tracks the review lifecycle independently of the collaboration store, and clean, failed, and dismissed duels close their conversation terminally.
