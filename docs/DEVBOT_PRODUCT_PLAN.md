...
- Discord slash commands:
  ...
  - `/queue add|list|remove|clear|start|stop|digest` stacks tasks for sequential, unattended execution through the same Ask/Do path. Each item is atomically claimed (queued -> running) before any execution side effect, so a concurrent remove or a crash mid-run can never double-execute or resurrect it; removal/clear/digest operate on stable item ids rechecked against the caller's current project access, execution re-verifies the original requester still has project access (fail closed), and digests are posted per-project to that project's own audience-safe room with a persisted digested marker so a later drain never re-reports old work. Runner state is durable and resumes cleanly across a restart.
  - `/schedule add|approve|revoke|list|remove|pause|resume` runs recurring owner-only tasks on a simple `daily HH:MM` / `weekdays HH:MM` / `every <N>h` spec. Schedules are read-only by default and by omission; `mode:action` is an explicit choice, and even then an occurrence never executes writes unattended: it either posts an approval-gated proposal card in the project's scoped workroom (one card per occurrence, durably linked to the occurrence, with later occurrences skipping while that card is undecided), or runs under a standing approval granted via `/schedule approve` with a mandatory expiry, max-run budget, and review checkpoint. Standing-approval runs atomically consume one run from the persisted budget before executing and post the proof-first completion card (with a revoke control) to the scoped audience; expiry, budget exhaustion, a reached review checkpoint, or revocation all fail closed back to proposal cards. Each tick atomically claims (leases) every due entry before resolving it, so an occurrence still running past its 30-second check-in is never claimed again by a later tick; checked on a 30-second loop with restart-safe next-run math and a recoverable running lease if Devbot restarts mid-occurrence. Unknown-mode schedule records are rejected on load and again at execution (fail closed), and safe mode blocks creating or resuming schedules, blocks granting standing approvals, and forces due action occurrences back to proposal cards.
