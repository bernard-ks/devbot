...
async function executeStandingScheduleRun(
  ...
  const result = await runProjectRequest({
    ...
    requesterId: entry.addedById,
    source: `schedule:${entry.id}`,
    // A standing-approval occurrence spawns a detached worker just like a read-only
    // one, so persist its identity before it starts working. This keeps action
    // schedules under the same crash-recovery double-execution protection: a later
    // boot can verify whether this specific worker survived before releasing the
    // occurrence, instead of treating action runs as a loophole around it.
    onWorkerStarted: (worker) => scheduleStore.recordRunningWorker(entry.id, worker),
    onProgress: async (progress) => {
      const content = `${formatTaskProgress(progress)}\n\n${trigger}`;
      const components = [taskControlRow(progress.taskId, { status: taskStatusForProgress(progress), mode: progress.mode })];
...
function commandRequiresController(interaction: ChatInputCommandInteraction): boolean {
  if (interaction.commandName === "do" || interaction.commandName === "run" || interaction.commandName === "ship" || interaction.commandName === "queue") {
    return true;
  }
