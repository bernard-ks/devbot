...
interface ParsedMentionRequest {
  project: ProjectEntry;
  text: string;
  includePatterns: string[];
}

function parseMentionRequest(content: string, botUserId: string, projects: ProjectEntry[]): ParsedMentionRequest {
  ...
  return { project, text, includePatterns };
}
