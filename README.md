...
- `/schedule add spec:<text> task:<text> project:<optional>`, `/schedule list`, `/schedule remove id:<id>`, `/schedule pause id:<id>`, `/schedule resume id:<id>`: Owner-only **read-only** recurring tasks (always `ask` mode — schedules fire unattended, so they can never write) using `daily HH:MM`, `weekdays HH:MM`, or `every <N>h` specs. Each occurrence is claimed atomically before it runs, so an entry still in flight past its 30-second check-in can never fire twice; checked every 30 seconds and restart-safe. While safe mode is on, new schedules cannot be created and paused schedules cannot be resumed.
