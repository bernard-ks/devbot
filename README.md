...
- `/clip project:<optional> target:<text> steps:<optional>`: Record a short local UI flow (up to ~4 scored interactions) and attach it as a video, transcoding to MP4 with `ffmpeg` when available and falling back to a screenshot if the recording is too large. Recording is deliberately read-only: steps can navigate document links, scroll, and hover, but never click buttons, submit forms, or toggle app state. Link steps load the anchor's vetted same-origin target directly instead of dispatching a DOM click, so a link's own `onclick` handler never runs. The whole browser session is confined to approved project origins (including redirects, popups, WebSockets, and service workers) and is cut off by a hard time budget.
...
- `/do task:<text> project:<optional> include:<optional patterns> watch:<optional, default false>`: Ask local Codex to perform a focused project task. Requires owner or controller access. Action tasks run in an isolated worktree that the running dev server does not serve, so automatic completion recording is skipped with an honest note pointing to `/clip` after merge (a recording would only show the unchanged source app). Opt in with `watch:true` to live-update the task message with dev-server screenshots every ~20s while it works (non-overlapping captures, last 12 frames stitched into a timelapse GIF on completion).
