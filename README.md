...
- Per-user and channel-wide rate limits (2 reports/hour/user, 10 reports/hour/channel) apply before anything else and persist across restarts; over-limit messages get a quiet reaction, not a reply. A reporter's reply to Devbot's own "need more detail" prompt continues the same report instead of spending another rate-limit slot.
