...
- `/preview share project:<optional> minutes:<optional 1-60>`: Owner-only. Detect a running local dev server for the project, independently re-validate it as an explicit loopback origin (exact scheme/host/port, not just a port number), and post a pre-spawn confirmation showing that origin, the project, git branch/revision, the TTL, and the anonymous-public disclosure with Confirm/Cancel buttons. Nothing is spawned until confirmed; an unconfirmed request auto-cancels after two minutes. Refuses when preview tunnels are disabled globally or for the project, `cloudflared` is missing, or no dev server is running.
- `/preview stop project:<optional>`: Owner-only. Stop an active or pending preview tunnel early (SIGTERM, escalating to SIGKILL if it doesn't exit). Works even while preview tunnels are disabled.
- `/preview status`: Owner-only. List active preview tunnels, their public URL, and the local origin they forward to. Works even while preview tunnels are disabled.
