...
- `/queue add task:<text> project:<optional> mode:<ask|do>`, `/queue list`, `/queue remove id:<id>`, `/queue clear`, `/queue start stop-on-failure:<optional>`, `/queue stop`, `/queue digest`: Stack tasks before bed and run them one at a time through the same Ask/Do path, then post one digest per project to that project's own audience-safe room when the queue drains. Every item is claimed atomically before it runs (no double-execution), removal/clear/digest are rechecked against your current project access and addressed by stable id (not a renumbered position), and each occurrence is re-authorized against the original requester right before it executes. Requires owner or controller access, and survives a restart.
- `/schedule add spec:<text> task:<text> mode:<ask|action> project:<optional>`, `/schedule approve id:<id> expires-in-hours:<n> max-runs:<n> review-after-runs:<optional> review-after-hours:<optional>`, `/schedule revoke id:<id>`, `/schedule list`, `/schedule remove id:<id>`, `/schedule pause id:<id>`, `/schedule resume id:<id>`: Owner-only recurring tasks using `daily HH:MM`, `weekdays HH:MM`, or `every <N>h` specs. Schedules are **read-only by default**: omitting `mode` always creates an `ask` schedule, and only the explicit `mode:action` choice creates a write-capable one. An `action` occurrence never executes on its own — it posts an approve/edit/decline proposal card to the project's scoped workroom (exactly one card per occurrence; while that card is undecided, later occurrences skip instead of stacking more), and work only starts once a controller approves that card. `/schedule approve` grants a **standing approval** for a specific action schedule with a mandatory expiry, a mandatory max-run budget, and a mandatory review checkpoint (after N runs and/or T hours); occurrences it covers execute without a fresh card, atomically consume one run from the budget, and post the same proof-first completion card (with a revoke button) to the scoped audience. Once the approval expires, exhausts its budget, hits a review checkpoint, or is revoked (via `/schedule revoke` or the button), occurrences fall back to proposal cards — never silent execution. Each occurrence is claimed atomically before it runs, so an entry still in flight past its 30-second check-in can never fire twice; checked every 30 seconds and restart-safe. While safe mode is on, new schedules cannot be created, paused schedules cannot be resumed, standing approvals cannot be granted, and due action occurrences fall back to proposal cards instead of executing.
