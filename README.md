...
- **Voice (opt-in, off by default):** set `DEVBOT_VOICE_ENABLED=true` to have Devbot transcribe a Discord voice message (or audio file) sent in the private room locally with ffmpeg and whisper.cpp, then offer Ask, Make change, and Dismiss buttons on the transcript. Voice notes carry their audio as Message Content data, so this path requires the privileged **Message Content Intent**: enable **MESSAGE CONTENT INTENT** under your application → Bot → Privileged Gateway Intents in the [Discord Developer Portal](https://discord.com/developers/applications) and set `DEVBOT_MESSAGE_CONTENT_INTENT=true`. Without it, Discord delivers content only for [documented exceptions](https://docs.discord.com/developers/events/gateway#message-content-intent) (DMs, messages that @mention the bot, the bot's own messages, and context-menu targets), so an ordinary voice note in the room arrives with no attachment. Enabling voice without the intent is refused with a setup message.
- **Set up:** `/setup wizard` is owner-only and resumable; `/setup doctor` diagnoses the full path, including the Message Content Intent prerequisite and local ffmpeg/whisper.cpp/model detection for voice notes.
