...
- **Voice (opt-in, off by default):** set `DEVBOT_VOICE_ENABLED=true` to have Devbot transcribe a Discord voice message (or audio file) sent in the private room locally with ffmpeg and whisper.cpp, then offer Ask, Make change, and Dismiss buttons on the transcript.
- **Set up:** `/setup wizard` is owner-only and resumable; `/setup doctor` diagnoses the full path, including local ffmpeg/whisper.cpp/model detection for voice notes.
