...
- `/setup preview action:<enable|disable> project:<optional>`: Owner-only public preview tunnels via Cloudflare Quick Tunnels — **anonymous, unauthenticated, public-Internet, testing-only links** with no Discord allowlist in front of them once shared. Off by default at two independent gates: omit `project` to flip the global kill-switch (also stops every active/pending tunnel), or pass `project` to allow/deny one project specifically (default deny; this is owner-controlled runtime state, not read from checked-in `.devbot/project.json`, so a repo contributor cannot grant themselves exposure by editing config).
- `/preview share project:<optional> minutes:<optional 1-60>`: Owner-only. Detect a running local dev server for the project, independently re-validate it as an explicit loopback origin (the exact scheme, address, and port are preserved, never rewritten), and post a pre-spawn confirmation showing that origin, the project, git branch/revision, the TTL, and the anonymous-public disclosure with Confirm/Cancel buttons; tunnel URLs are always posted embed-suppressed (`<...>` plus the suppress-embeds flag) so the control message itself does not trigger a crawler. Nothing is spawned until confirmed; an unconfirmed request auto-cancels after two minutes. Refuses in safe mode, when preview tunnels are disabled globally or for the project, when `cloudflared` is missing, or when no dev server is running.
- `/preview stop project:<optional>`: Owner-only. Stop an active or pending preview tunnel early (SIGTERM, escalating to SIGKILL if it doesn't exit, reported honestly if neither confirms). Works even while preview tunnels are disabled or in safe mode.
- `/preview status`: Owner-only. List active preview tunnels, their public URL, and the local origin they forward to. Works even while preview tunnels are disabled or in safe mode.
