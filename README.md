...
- `/review duel task:<task-id>` (also a **Duel review** button on completed action tasks): Have an independent Codex session, on a different Luna/Terra/Sol tier than the author, adversarially review evidence pinned to the task's recorded base revision (not just the mutable working tree), give an author-side session one concede/rebut round, then post the resolved issue list (conceded / disputed), evidence coverage, parser warnings, and reviewer-independence status to a dedicated thread. At duel time the exact reviewed state is also pinned as a git commit under a hidden `refs/devbot/duels/<duel-id>` ref (same sensitive-path exclusions as the evidence, working tree untouched). Conceded issues get owner/controller decision controls bound to the duel thread: **Accept & fix** appears only while that snapshot ref still resolves to its recorded commit and tree, atomically claims the acceptance on the durable duel record, seeds a fresh isolated worktree from the snapshot (never the source checkout or the original task worktree), creates the fix task through the normal task path with the conceded issues as its input, and records the resulting task ID — exactly one launch even under duplicate submissions, with failures released as retryable. If the snapshot is missing or stale, the row falls back to **Copy fix prompt** (a follow-up prompt to paste into `/do` yourself) with a truthful explanation; **Dismiss** closes the duel. Safe mode blocks acceptance, and snapshot refs are cleaned up when their duels are pruned, dismissed, or accepted.
