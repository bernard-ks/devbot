...
## Review round 1 (bernard, `REVIEW.md`)

Addressed every numbered blocking issue. `src/tunnel.ts` and `src/tunnel-ui.ts` were substantially redesigned; `src/index.ts`'s preview command/button handling was rewritten around the new pending -> active -> stopping lifecycle. All changes are new commits on top of `f13409d` (history not rewritten). `npm test`: 157/157 green (no rerun needed this time).

1. **Do not publish an unauthenticated raw dev origin** — `/preview share` no longer spawns anything directly. It reserves a pending tunnel and posts a pre-spawn confirmation (`tunnelConfirmMessage`, `src/tunnel-ui.ts`) showing the exact local origin, project, git branch/revision (`describeProjectRevision`, `src/tunnel.ts`), TTL, and an explicit "anonymous public Internet URL / Cloudflare Quick Tunnel / testing-only, no Discord allowlist" disclosure, with Confirm/Cancel buttons. Nothing spawns until the owner clicks Confirm (`handlePreviewConfirmButton`, `src/index.ts`); an unconfirmed request auto-cancels after `PENDING_CONFIRM_TIMEOUT_MS` (2 min). Both the confirmation and the final share message set `MessageFlags.SuppressEmbeds` to keep Discord from unfurling the URL. Tests: `tunnelConfirmMessage discloses the exact origin...` (`tunnel-ui.test.ts`), `TunnelManager pending confirmation auto-expires...` (`tunnel.test.ts`).
2. **Bind the tunnel to the exact validated loopback origin** — new `validateLoopbackOrigin()` (`src/tunnel.ts`) independently re-validates every candidate URL regardless of what `findProjectWebUrls` already filtered: rejects remote hosts and credentials, preserves scheme and explicit/default port, normalizes `localhost`/`[::1]` to `127.0.0.1`. `findRunningProjectOrigin()` (renamed from `findRunningProjectPort`) only accepts validated origins and returns the full origin string, which cloudflared is launched against directly (`--url <origin>`) instead of always reconstructing `http://127.0.0.1:<port>`. Tests: `validateLoopbackOrigin preserves scheme and port...`, `findRunningProjectOrigin independently re-validates candidates, ignoring anything upstream missed` (proves a "reachable" remote candidate is still rejected).
3. **Enforce project authorization and audience scope** — `handlePreviewCommand`'s share path now calls `ensureProjectAccess()` (recheck of the per-project allowlist, same as `/status`/`/snip`), respects project-room binding (`boundProjectNameForRoom`; refuses with `tunnelWrongRoomMessage` if the room is bound to a different project), and replies ephemeral when `hasProjectAudienceRestriction(project)` — identical pattern to existing commands. A new default-deny per-project preview policy (`previewEnabledProjectNames` on `AppConfig`, backed by `setupStore.previewEnabledProjects`) is owner-controlled runtime state, never read from checked-in `.devbot/project.json`; `/setup preview action:enable project:<name>` is required in addition to the global switch. Owner `/preview stop`/`status` deliberately skip these checks (see #5).
4. **Race-safe pending/active/stopping state** — `TunnelManager` now has `reserve()` (synchronous, claims the project slot and a global concurrency slot before anything spawns) and `launch()` (async spawn against a confirmed reservation). Two concurrent share requests for the same project can never both spawn: the second `reserve()` throws synchronously. A global `MAX_CONCURRENT_TUNNELS` (3) caps pending+active tunnels together. Pending reservations are abortable via `cancelPending()`/`stopByProject()`, including mid-spawn (the child is attached to the reservation the instant it is spawned, before awaiting its URL, so a cancel that lands during that wait kills it immediately). Tests: `reserve claims the project slot synchronously...`, `reserve enforces a global concurrency limit...`, `launch aborts and cleans up when the reservation is cancelled while cloudflared is starting`.
5. **Disabling stops live tunnels; stop/status stay available** — `/setup preview action:disable` (global or per-project) now calls `tunnelManager.stopAll("disabled")` / `stopByProject(name, "disabled")` and edits every affected message. `handlePreviewCommand` now checks the enablement flag only for `share`; `stop` and `status` only require `previewOwnerGateReason` (owner identity), independent of the feature flag or project policy, so the owner can always kill/inspect what is already running. Test: `previewOwnerGateReason and previewGateReason report why /preview would be refused` (the two gates are now separate exported functions).
6. **Confirmed-exit lifecycle** — `stop()`/TTL expiry now transition through a `"stopping"` state, send SIGTERM, wait up to `killGraceMs` (default 5s) for a real `exit` event, escalate to SIGKILL and wait again if needed, and only then remove the tunnel and fire `onExpire`. If neither signal produces an observed exit, it logs a clear "did not confirm exit... check for an orphaned process manually" warning instead of claiming success silently. Tests: `stop kills the child with SIGTERM, waits for confirmed exit...`, `stop escalates to SIGKILL when cloudflared ignores SIGTERM`, `stop gives up after SIGTERM and SIGKILL both go unanswered, without hanging forever`.
7. **Fail closed on child-process and publish errors** — `waitForTunnelUrl`/`startCloudflaredTunnel` now handle the child's `error` event (spawn failures reject cleanly instead of throwing as an unhandled EventEmitter error), and a post-startup `error` is treated the same as an unexpected exit (still attempts a kill, since `error` doesn't guarantee the process died — see the `alreadyExited` distinction in `finalizeActive`). `handlePreviewConfirmButton` rechecks `tunnelManager.get(tunnel.id)` before publishing (guards a concurrent stop/shutdown race) and, if publishing the share message itself throws, stops the just-launched tunnel and logs rather than leaving it live with no visible control. Every tunnel gets a random `id` (not keyed by project name); attach/stop/edit all address by that id. Tests: `startCloudflaredTunnel rejects on a spawn error and swallows a later error without crashing`, `TunnelManager treats a post-startup child error the same as an unexpected exit, without crashing`.
8. **Controls bound to a tunnel instance** — `tunnelControlRow`/`parseTunnelControl` and the new `tunnelConfirmRow`/`parsePreviewConfirmControl` all key off the random tunnel id, not the project name. `handlePreviewStopButton`/`handlePreviewConfirmButton` verify the id still resolves to a tracked (pending/active) tunnel before acting and reply with `tunnelControlExpiredMessage()` for a stale control instead of silently acting on whatever is currently active for that project. Tests: `parseTunnelControl accepts well-formed stop customIds keyed by tunnel id...`, `parsePreviewConfirmControl accepts start/cancel customIds keyed by tunnel id...`, `TunnelManager get/getPending report undefined for stale (unknown) ids`.
9. **Isolate cloudflared from ambient Cloudflare state** — `TunnelManager.launch()` creates a single-use temp directory (`mkdtemp` under `os.tmpdir()`, same convention as `command-runner.ts`'s per-run `HOME`) and spawns cloudflared with `HOME`/`USERPROFILE` overridden to it on top of `minimalChildEnvironment()`, so it cannot read `~/.cloudflared` or receive bot secrets. The directory is removed once the tunnel is confirmed stopped. Test: `TunnelManager launch spawns with an isolated HOME, no bot secrets, and the exact validated origin` (asserts `HOME` differs from the real one, `DISCORD_TOKEN` is absent, and — mirroring `security.test.ts`'s "empty temporary home" pattern — that the directory no longer exists after removal).
10. **End-to-end evidence and accurate docs** — added spawned-fake-cloudflared and pure-logic coverage for all of the above (30 tests in `tunnel.test.ts`, 8 in `tunnel-ui.test.ts`, plus a new `setup-store.test.ts` case for the per-project allow-list). README.md and `docs/DEVBOT_PRODUCT_PLAN.md` now state plainly that Quick Tunnels are anonymous, unauthenticated, public-Internet, testing-only links not protected by Discord allowlists, and document the two-gate enablement, pre-spawn confirmation, exact-origin binding, and isolated-HOME behavior. `/setup doctor` now reports whether `cloudflared` is present on PATH when preview tunnels are enabled (existence check only, via the existing non-spawning `findCloudflaredPath` PATH scan — deliberately not a `cloudflared --version` spawn, to honor "never actually spawn cloudflared" outside the real bot process).

### Known gaps carried forward
- The per-project preview policy is a flat allow-list (no `approval` tier like `screenshotPolicy`); adding one would be straightforward if bernard wants it.
- Global `MAX_CONCURRENT_TUNNELS` (3) and `PENDING_CONFIRM_TIMEOUT_MS` (2 min) are constants, not owner-configurable; can be wired to `/setup preview` if desired.
- If a confirmation message can't be published at all (e.g. the interaction expires before `editReply`), the reservation still self-expires via `PENDING_CONFIRM_TIMEOUT_MS` — there's no separate rollback path for that specific case, since nothing has been exposed yet.
